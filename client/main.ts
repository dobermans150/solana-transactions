...
async function main() {
    ...
    await sendLamports(johnKeypair, ringoKeypair.publicKey, LAMPORTS_PER_SOL * 0.004);
    ...
    await sendLamports(paulKeypair, georgeKeypair.publicKey, LAMPORTS_PER_SOL * 0.000023);

    // George sends some SOL over to John.
    console.log("George sends some SOL over to John...");
    console.log(`   George's public key: ${georgeKeypair.publicKey}`);
    console.log(`   John's public key: ${johnKeypair.publicKey}`);
    await sendLamports(georgeKeypair, johnKeypair.publicKey, LAMPORTS_PER_SOL * 0.00000012);
}
